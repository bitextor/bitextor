...
(
snakemake --snakefile "${BITEXTOR}/snakemake/Snakefile" --notemp --configfile "${BITEXTOR}/snakemake/example/tests/only-preprocess-greenpeace.yaml" --config bitextor="${BITEXTOR}" lang1='en' lang2='fr' permanentDir="${WORK}"/permanent/bitextor-output-preprocess-greenpeace/ dataDir="${WORK}"/data transientDir="${WORK}"/transient-greenpeace preprocessLangs="en,fr" parser="alcazar" boilerpipeCleaning=True -j 4 2> "${WORK}"/reports/preprocess-greenpeace.report
