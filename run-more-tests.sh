...
(
snakemake --snakefile "${BITEXTOR}/snakemake/Snakefile" --configfile "${BITEXTOR}/snakemake/example/tests/only-preprocess-kremlin.yaml" --config bitextor="${BITEXTOR}" lang1='en' lang2='ru' permanentDir="${WORK}"/permanent/bitextor-output-preprocess-kremlin dataDir="${WORK}"/data transientDir="${WORK}"/transient-kremlin wordTokenizers="{'en': '${BITEXTOR}/preprocess/moses/tokenizer/tokenizer.perl -q -b -a -l en', 'ru': '${BITEXTOR}/preprocess/moses/tokenizer/tokenizer.perl -q -b -a -l ru'}" sentenceSplitters="{'en': '${BITEXTOR}/preprocess/moses/ems/support/split-sentences.perl -q -b -l en', 'ru': '${BITEXTOR}/preprocess/moses/ems/support/split-sentences.perl -q -b -l ru'}" targetLangs="ru" parser="alcazar" -j 4 2> "${WORK}"/reports/preprocess-kremlin.report
