...
for nmodel in xrange(0,options.nbest):
  ...
  checkpoint=ModelCheckpoint(options.weights, monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=True, mode='auto', period=1)


  sgd = SGD(lr=0.5, decay=1e-4, momentum=0.1, nesterov=True)
  model.compile(loss='binary_crossentropy',  optimizer=sgd)

  trainingout=model.fit(feats_array_train, labels_array_train, batch_size=128, nb_epoch=10000, verbose=1, callbacks=[earlyStopping,checkpoint], validation_data=(feats_array_dev,labels_array_dev), shuffle=True, class_weight=None, sample_weight=None)
