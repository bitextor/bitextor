...
oparser = argparse.ArgumentParser(
    description="Script that takes the output of bitextor-crawl2ett and removes duplicate files.")
...
oparser.add_argument("-l", "--languages",
                     help="List accepted languages represented as a comma separated language codes list",
                     dest="langlist", default=None)

options = oparser.parse_args()

langs = []
if options.langlist is not None:
...
for line in sys.stdin:
    ...
    if lang in langs:
        ...
        if c.hexdigest() in seen_md5:
            ...
            print("\t".join(pageToks[0:4]) + "\t" + base64.b64encode(html_text.encode("utf-8")).decode(
                "utf-8") + "\t" + base64.b64encode(text.encode("utf-8")).decode("utf-8") + "\n")
