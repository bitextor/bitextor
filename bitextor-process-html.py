...
#
# 1. This script reads a tab-separated list of documents, only containing two fields: the content of the document
# encoded with base64 and the URL
...
# Inline tags that don't start on a new line and only take up as much width as necessary.
# From https://www.w3schools.com/html/html_blocks.asp
...
def get_element_text(element, document):
    ...
    for child in element:
        if type(child) is etree._Element and child.tag != "script" and child.tag != "style":
            text = text + get_element_text(child, document)
    ...
    if element.tail is not None:  # If we have tail parent text (text after the closing tag until the next
        # open/close tag)
...
def get_document_text(document):
    ...
    for element in document.getroot():
        if type(
                element) is etree._Element and element.tag != "script" and element.tag != "style":  # Only elements,
            # not scripts or other kind of tags without proper text
            docplaintext = docplaintext + get_element_text(element, document)
    return docplaintext
...
oparser = argparse.ArgumentParser(
    description="Script that takes the output of bitextor-crawl and adds to the list of fields the MIME type and the"
                "character encoding detected.")
oparser.add_argument('--input', dest='input', help='Input HTML file')
oparser.add_argument('--metadata', dest='metadata',
                     help='Input file containing the URL and crawling time for the input file')
...
oparser.add_argument('--text', dest='text',
                     help='Output file with plain text from normalized HTML without boilerplates')
...
with open("{metadata}".format(metadata=options.metadata), "rt") as metadata:
    ...
    if len(fields) >= 1:
        ...
        file.write(
            lang + "\t" + mimeEncode[0] + "\t" + mimeEncode[1] + "\t" + url + "\t" + str(filename).split(".")[0] + "\n")
        file.close()
