...
namespace align {
    ...
    void AlignDocuments(const std::string &output_path, const utils::AlignData &align_data, const StringPiece matched_text1_uri,
      ...
      const VecStr *text1 = nullptr, *text2 = nullptr;

      utils::umap_extracted::const_iterator iter;
    
      iter = align_data.umap_text1.find(matched_text1_uri.data());
      if (iter != align_data.umap_text1.end()) {
        text1 = &iter->second;
      }

      iter = align_data.umap_text2.find(matched_text2_uri.data());
      if (iter != align_data.umap_text2.end()) {
        text2 = &iter->second;
      }

      if (text1 && text2) {
          Align(matches, *text1, *text2, threshold);

          WriteAlignedTextToFile(output_path, matches, *text1, *text2);
      }

    }

    void Align(utils::matches_vec &matches, const std::vector<std::string> &text1_doc,
               const std::vector<std::string> &text2translates_doc, double threshold) {

      std::vector<utils::scoremap> scorelist;
      EvalSents(scorelist, text2translates_doc, text1_doc, 2, 3);
      search::FindMatches(matches, scorelist, text2translates_doc.size(), text1_doc.size(), threshold);
      GapFiller(matches, text2translates_doc, text1_doc, 3, threshold);
    }

    /* given list of test sentences and list of reference sentences, calculate bleu scores */
    void EvalSents(std::vector<utils::scoremap> &scorelist, const std::vector<std::string> &text2translates_doc,
                   const std::vector<std::string> &text1_doc, unsigned short ngram_size, size_t maxalternatives) {

      std::vector<ngram::NGramCounter> cooktarget;
      for (size_t i = 0; i < text1_doc.size(); ++i) {
        scorer::cook_ref_set(cooktarget, text1_doc.at(i), ngram_size);
      }

      for (size_t trans_i = 0; trans_i < text2translates_doc.size(); ++trans_i) {
        // copied over from bleu.py to minimize redundancy
        std::vector<std::string> test_normalized;
        scorer::normalize(test_normalized, text2translates_doc.at(trans_i), "western");
    ...
    void GapFiller(utils::matches_vec &matched, const std::vector<std::string> &text2translates_doc,
                   const std::vector<std::string> &text1_doc, size_t gap_limit, double threshold) {
      ...
      std::unique_ptr<int[]> matches_arr_translated = boost::make_unique<int[]>(text2translates_doc.size());
      std::unique_ptr<int[]> matches_arr_text1 = boost::make_unique<int[]>(text1_doc.size());
      std::fill(matches_arr_translated.get(), matches_arr_translated.get() + text2translates_doc.size(), -1);
      std::fill(matches_arr_text1.get(), matches_arr_text1.get() + text1_doc.size(), -1);


      for (auto m: matched) {
        matches_arr_translated[m.first.from] = m.second.from;
        matches_arr_text1[m.second.from] = m.first.from;
      }

      std::vector<std::string> merged_text_translated;
      utils::vec_pair merged_pos_translated;
      std::vector<std::string> merged_text_text1;
      utils::vec_pair merged_pos_text1;

      for (auto &m: matched) {
        for (int post = 0; post < 2; ++post) {

          if (post == 0) { // pre
            PreGapMergedSentences(merged_text_translated, merged_pos_translated, text2translates_doc,
                                  matches_arr_translated, m.first.from, gap_limit);
            PreGapMergedSentences(merged_text_text1, merged_pos_text1, text1_doc,
                                  matches_arr_text1, m.second.from, gap_limit);
          } else if (post == 1) { // post
            PostGapMergedSentences(merged_text_translated, merged_pos_translated, text2translates_doc,
                                   matches_arr_translated, text2translates_doc.size(), m.first.from, gap_limit);
            PostGapMergedSentences(merged_text_text1, merged_pos_text1, text1_doc,
                                   matches_arr_text1, text1_doc.size(), m.second.from, gap_limit);
          }

          if (merged_text_translated.size() == 1 && merged_text_text1.size() == 1)
            continue;

          std::vector<utils::scoremap> scorelist;
          EvalSents(scorelist, merged_text_translated, merged_text_text1, 2, 3);

          // find max
          float max_val = -1;
          size_t max_pos_translate;
          size_t max_pos_text1;
          for (size_t i = 0; i < scorelist.size(); ++i) {
            ...
            if (scorelist.at(i).rbegin()->first > max_val && scorelist.at(i).rbegin()->first > threshold) {
              ...
              max_pos_translate = i;
              max_pos_text1 = scorelist.at(i).rbegin()->second.first;
            }
          }
          ...
          FillMatches(matches_arr_translated, matches_arr_text1, m);
