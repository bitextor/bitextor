...
# 2. Reads RIDX file
# 3. Uses a regressor to combine all the scores (features) assigned to each pair of documents and obtain a similarity
# metric that combines all of them
...
oparser = argparse.ArgumentParser(
    description="Script that rescores the aligned-document candidates provided by script bitextor-idx2ridx by using the"
                "Levenshtein edit distance of the structure of the files.")
oparser.add_argument('ridx', metavar='RIDX', nargs='?',
                     help='File with extension .ridx (reverse index) from bitextor-idx2ridx (if not provided, the'
                          'script will read from the standard input)',
                     default=None)
oparser.add_argument("-w", "--weights",
                     help="File where the weights of the model to be used for document alignment are stored",
                     dest="weights", required=True)
oparser.add_argument("-m", "--model",
                     help="File where the architecture of the model to be used for document alignment is stored",
                     dest="model", required=True)
oparser.add_argument("-t", "--threshold", help="Threshold for the score assigned to each candidate", dest="threshold",
