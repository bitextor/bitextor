...
def print_tu(urlstring1, urlstring2, restofdata):
    numid = 0
    for segid in restofdata.keys():
        ...
        if 'numTokensSL' in fieldsdict and fieldsdict['numTokensSL'] != "" and 'numTokensTL' in fieldsdict and \
                fieldsdict['numTokensTL'] != "":
            print("    <prop type=\"lengthRatio\">" + str(
                round(float(fieldsdict['numTokensSL']) / float(fieldsdict['numTokensTL']), 4)) + "</prop>")
...
print_tu(segdicturls1, segdicturls2, segdictotherdata)
print(" </body>")
print("</tmx>")
reader.close()
