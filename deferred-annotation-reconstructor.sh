...
zcat -f $1 | while read line; do
	...
	if [ ! -f $tempsplitsent1 ]; then
	        if [ "$4" != "" ]; then
			> ${tempwarc}.warc.gz
			for id in $(warcio index $4 | grep -i "$url1" | cut -f 1 -d ',' | cut -f 2 -d ':' | cut -f 2 -d '"'); do warcio extract $4 $id | gzip >> ${tempwarc}.warc.gz; done
		...
		zcat $tempprocess/*/text.gz | python3 bitextor-split.py --langcode $2 > $tempsplitsent1
	fi
	...
	if [ ! -f $tempsplitsent2 ]; then
		if [ "$5" != "" ]; then
			> ${tempwarc}.warc.gz
			for id in $(warcio index $5 | grep -i "$url2" | cut -f 1 -d ',' | cut -f 2 -d ':' | cut -f 2 -d '"'); do warcio extract $5 $id | gzip >> ${tempwarc}.warc.gz; done
		else
		...
		zcat $tempprocess/*/text.gz | python3 bitextor-split.py --langcode $3 > $tempsplitsent2
	fi
