...
def extract_encoded_text(encodedtext, outfile, sent_tokeniser, word_tokeniser, morph_analyser):
...
oparser.add_argument('--lang', dest='lang',
                     ...
                     required=True)
oparser.add_argument("--output-dir", dest="output_dir", default=".", help="Output directory. Output files will be named as <lang>.tokenized.xz", required=False)
...
oparser.add_argument("--sent-tokeniser_sl", help="Path to the sentence tokeniser for SL", dest="senttok1", required=True)
oparser.add_argument("--sent-tokeniser_tl", help="Path to the sentence tokeniser for TL", dest="senttok2", required=True)
...
outfile = lzma.open(os.path.join(options.output_dir, "plain_tokenized.xz"), "wb")
with open_xz_or_gzip_or_plain(options.text) as text_reader, \
    ...
    for line in text_reader:
        ...
        if lang == options.lang1:
            extract_encoded_text(encodedtext, outfile, options.senttok1, options.wordtok1, options.morphtok1)
        elif lang == options.lang2:
            extract_encoded_text(encodedtext, outfile, options.senttok2, options.wordtok2, options.morphtok2)
        else:
            outfile.write(encodedtext + b"\n")
